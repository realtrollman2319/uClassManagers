...
namespace uClassManagers
{
    ...
    public class BarricadeManagerClass
    {
        ...
        {
            BarricadeManager.changeOwnerAndGroup(barricade.BarricadeTransform, UInt64.Parse(newOwner), UInt64.Parse(newGroup));
        }
        ...
        public static BarricadeClass DropPlantedBarricade(ushort id, VehicleClass vehicle, Vector3Class localPosition, float rx, float ry, float rz, string owner = "0", string group = "0")
        {
            ...
            if (barricade.asset == null) return (BarricadeClass)null; // Code below this ScriptFunction belongs to Ster#0001. It just got modified a bit.
        ...
        */
        ...
        public static void SendStorageDisplay(BarricadeClass storage, ItemClass item, ushort skinID = 0, ushort mythicID = 0, string tagGiven = "", string dynamicPropsGiven = "")
        {
            BarricadeManager.sendStorageDisplay(storage.BarricadeTransform, item.Item.item, skinID, mythicID, tagGiven, dynamicPropsGiven);
        }
        ...
        */
        ...
        public static void UpdateReplicatedState(BarricadeClass barricade, ExpressionValue state)
        {
            ...
            foreach (var arrayElement in state.AsList)
            {
                byte parsedByte;
                ...
                {
                    Console.WriteLine("uClassManagers (BarricadeManager) module from uScript => One of the array elements failed to parse! Numbers must only be 0 to 255 and not a string or an object. Check the array elements and try again.", Console.ForegroundColor = ConsoleColor.Red);
                    Console.ResetColor();
                    return;
                }
            }

            byte[] newState = bList.ToArray();
            BarricadeManager.updateReplicatedState(barricade.BarricadeTransform, newState, newState.Length);
        }

        [ScriptFunction("updateState")]
        public static void UpdateState(BarricadeClass barricade, ExpressionValue state)
        {
            List<byte> bList = new List<byte>();
            ...
            byte[] newState = bList.ToArray();
            BarricadeManager.updateState(barricade.BarricadeTransform, newState, newState.Length);
        }
