    ...
    public class VehicleManagerClass
    {
        ...
        public static ExpressionValue Vehicles
        {
            get
            {
                ...
                {
                    list.Add(ExpressionValue.CreateObject(new VehicleClass(vehicle)));
                }
        ...
        {
            EDamageOrigin? origin = EnumTool.damageOrigins.GetEnum(damageOrigin);
            if (origin == null) return;
            VehicleManager.damage(vehicle.Vehicle, damage, times, canRepair, new CSteamID(ulong.Parse(instigatorSteamID)), origin.Value);
        }
        ...
        public static void DamageTire(VehicleClass vehicle, int tireIndex, string instigatorSteamID = "0", string damageOrigin = "unknown")
        {
            EDamageOrigin? origin = EnumTool.damageOrigins.GetEnum(damageOrigin);
            if (origin == null) return;
            VehicleManager.damageTire(vehicle.Vehicle, tireIndex, new CSteamID(ulong.Parse(instigatorSteamID)), origin.Value);
        }
